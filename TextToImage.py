...
# Sidebar previous prompts expander
with st.sidebar.expander("Previous Prompts", expanded=True):
    # Check if history exists in session_state
    if "history" in st.session_state:
        for entry in st.session_state.history:
            if st.button(entry[0]):
                selected_image = entry[1]
                st.image(selected_image, use_container_width=True)
    else:
        st.write("No previous prompts to display.")
            ...
            try:
                ...
                if not api_key:
                    ...
                    if response.status_code == 200:
                        ...
                        # Store the prompt and image in session state for future use
